...
exports.updateDoctor = async (req, res, next) => {
    try {
      ...
      if (uniqueFields) {
        switch (true) {
          case uniqueFields.email === email:
            return res.status(401).json({ message: "This email is being used by another Doctor" });
          case uniqueFields.phone === phone:
            return res.status(401).json({ message: "This phone is being used by another Doctor" });
          case uniqueFields.card === card:
            return res.status(401).json({ message: "This card is being used by another Doctor" });
          default:
      ...
      return res.status(200).json({ message: "Update doctor successfully", doctor });
    } catch (err) {
